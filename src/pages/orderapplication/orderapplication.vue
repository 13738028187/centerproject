...
<div class="container">
    ...
    <button type="warn" class="sub1" @tap="toCheckno(orderdetail.sid)">审核失败</button>
    <button type="primary" class="sub2" @tap="toCheck(orderdetail.sid)">审核通过</button>
    ...
    methods: {
      ...
      toCheckno(id){
          if(this.message!=''&&this.message!=null){
            this.$fly.request({
              method:"post", //post/get 请求方式
              url:this.$http.state.orderappno,
              body:{sid:id,reason:this.message}
                ...
                this.$fly.request({
                    ...
                    body:{appId:'wx4d0eaf46c3283887',ordercode:this.orderdetail.paycode,total_fee:0.01,refund_fee:0.01}
                    }).then(res =>{
                        wx.reLaunch({url: "/pages/index/main"})
                })  
            })
