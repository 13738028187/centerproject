...
<div class="container">
    ...
    <van-collapse :value="activeName" @change="onChange($event)" custom-class="collapses" accordion>
      ...
      <div v-show="showrec">
      <van-collapse-item title="器械回收" name="4" >
            <van-cell-group custom-class="changegroups" >
                <div class="headimage">
                      <scroll-view scroll-x style="width: 100%" >
                        <div  style="display:flex;justify-content:center;">
                            <div v-for="(item,index) in recoryimgs" :key="index">
                            <image :src="item" style="width:250rpx;height:200rpx;margin-right:10rpx;margin-bottom:10rpx"></image>
                            </div>
                        </div>
                      </scroll-view>
      ...
      <div v-show="showfk">
        <van-collapse-item title="用户评价" name="5" >
            <div class="headimage">
                    <scroll-view scroll-x style="width: 100%" >
                      <div  style="display:flex;justify-content:center;">
                          <div v-for="(item,index) in fkimages" :key="index">
                          <image :src="item" style="width:250rpx;height:200rpx;margin-right:10rpx;margin-bottom:10rpx"></image>
                          </div>
                      </div>
                    </scroll-view>
          ...
          <van-cell-group custom-class="changegroups">
              <van-cell title="服务态度" :value="orderdetail.fkattitude" custom-class="changegroup"/>
              <van-cell title="专业技术" :value="orderdetail.fkskills" custom-class="changegroup"/>
              <van-cell title="亲和力" :value="orderdetail.fkaffinity" custom-class="changegroup"/>
              <van-cell title="反馈时间" :label="orderdetail.fkdateline" custom-class="changegroup"/>
              <van-cell title="反馈内容" :label="orderdetail.fkcontent" custom-class="changegroup"/>     
          </van-cell-group>
        </van-collapse-item>     
...
<script>
import { transition } from '../../../dist/wx/static/vant-weapp/mixins/transition';
  ...
  export default {
    ...
    onLoad(options){
        ...
        if(this.orderdetail.recimg){
              this.showrec=true
              this.recimgs=JSON.parse(this.orderdetail.recimg)
              this.recimgs.forEach((item,index)=>{
              this.recoryimgs.push("https://www.ty5m.com:8088/phy/upload/"+item.imgdate.substr(0,10)+'/'+item.imgroute)
            })
          }
         if(this.orderdetail.fkdateline){
           this.showfk=true
           if(this.orderdetail.fkimg){
              this.fkimgs=JSON.parse(this.orderdetail.fkimg)
              this.fkimgs.forEach((item,index)=>{
              this.fkimages.push("https://www.ty5m.com:8088/phy/upload/"+item.imgdate.substr(0,10)+'/'+item.imgroute)
            })
          }
         } 
