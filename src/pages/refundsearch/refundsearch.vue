        ...
        <container>
          <scroll-view scroll-y style="height: 1150rpx;" @scrolltolower="lower1">
            <div v-for="(item,index) in readylist" :key="index">
              <div class="servlist" @tap="item.pass==2?urlorderapplication(item.sid):urlorderdetail(item.sid)">
                  <div class="listleft"><image :src="item.images" @tap="taskuser"/>
                  </div>
                  <div class="listright">
                    <span class="biao">{{item.name}} 的服务申请</span>
                    <span class="user" v-show="item.pass==2">待退款</span>
                    <span class="user" v-show="item.refundflag==1">已退款</span>
                    <span class="user" v-show="item.refundflag==2">未退款</span>
